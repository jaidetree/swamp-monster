...
- [x] Owner can create/edit/delete a `Work` and its `WorkImage` gallery
      entries through Django admin.
- [x] Drag-reordering `Work` rows in the admin list persists the new
      `order` via the ported `renumber()` helper.
- [x] Drag-reordering `WorkImage` rows — including a newly added, unsaved
      row — persists the new `order`.
- [x] `published`/`featured` toggles are editable in admin and covered by
      model-level tests for `published`/`featured`/`order` filtering.
- [x] Markdown `description`/`caption` renders as HTML via
